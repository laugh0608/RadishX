...
| Mascot 虚拟形象页 | [mascot-page.md](../features/mascot-page.md) | 单张拆分与下载政策已评估，当前继续整图预览 |
| About 联系区 | [about-contact-surface.md](../features/about-contact-surface.md) | 首轮精细化已完成 |
| 素材治理 | [asset-governance.md](../features/asset-governance.md) | Mascot 表情 / 贴纸接入与拆分政策已记录 |
...
11. 目标文档、本文件和本周开发日志已同步记录。

下一步建议推进：

1. 继续评估 `seasonal/` 节日活动图的具体活动窗口、页面位置和文案；未确认活动目标前不进入长期页面。
2. 如后续决定推进 Mascot 单张展示，先建立拆分清单、首批候选和命名规则；没有确认版权 / 授权 / 文件包边界前不提供下载入口。
3. 四个项目后续只有出现新的独立 Logo、真实截图或视频候选来源时，再按素材治理文档重新审核。
4. 本地开发阶段继续以本地真实 smoke 为准；暂不推送、部署或补跑线上 smoke。
...
- “萝小白”后续是否启动单张表情拆分清单、节日活动图和下载口径。
- 项目详情模板、Mascot 页和 About 页是否在 React 实现前补独立移动版设计稿。

## 后续顺序

1. 继续评估 Mascot 节日活动图的活动窗口、页面位置和文案；确认前不进入长期 Gallery 或默认主视觉。
2. 如需推进 Mascot 单张表情展示，先建立拆分清单和首批候选；确认版权、授权和文件包边界前不提供下载入口。
3. 四个项目出现新的独立 Logo、真实截图或视频候选来源后，再回到项目素材审核和 Web 优化。
4. 后续只有首页视觉再次明显变化或社交预览目标变化时，再评估 Open Graph 分享预览图。
