...
12. 目标文档、本文件和本周开发日志已同步记录。

下一步建议推进：

1. 如果后续选择具体 seasonal 活动，再按已评估的窗口、位置、文案边界和授权要求生成 Web 版本并进入短期页面实现。
...
1. 如需使用 seasonal 图，先选择具体活动目标并确认上线 / 下线时间、页面位置、文案、授权和撤下策略，再生成 Web 版本。
2. 四个项目出现新的独立 Logo、真实截图或视频候选来源后，再回到项目素材审核和 Web 优化。
3. 后续只有首页视觉再次明显变化或社交预览目标变化时，再评估 Open Graph 分享预览图。
4. 如需推进 Mascot 单张表情展示，先建立拆分清单和首批候选；确认版权、授权和文件包边界前不提供下载入口。
