...
- 补齐官网第一版基础 metadata、canonical、Open Graph、Twitter Card、404 noindex、skip link 和外链安全口径，为 Vercel 上线前检查做准备。
...
3. 确认首页主视觉最终采用项目矩阵合成视觉、可爱Q版还是虚拟形象完全体素材，并为通过审核的素材生成 Web 优化版本。
4. 视审核结果补项目详情模板、Mascot 页和 About 页移动版设计稿，或在实现阶段按桌面稿继续拆响应式布局。
5. 继续做上线前浏览器 smoke，重点覆盖页面 metadata、站内路由、外链、404、首页主视觉状态槽、四个项目详情页状态区、Mascot 三形态和 About 联系区。
6. 根据 Vercel 首次部署结果复查站内路由 fallback、缓存、分享预览和域名绑定。
