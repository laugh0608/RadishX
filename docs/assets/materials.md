...
- “仅限本站展示”描述授权使用范围，不表示页面或资源仅内部可见。进入 `public/` 并随官网部署的资源可以被访客直接访问；关闭下载按钮不构成访问控制。
- 首批表情的 `internal-preview` 是沿用的历史状态值，表示官网内候选展示用途；本轮不修改数据字段或扩大允许页面位置。
- 公开文案优先使用“本站展示候选”“仅供本站展示，未开放复用授权”。需要审批的新增候选仍需先审核，不能因术语调整而直接进入页面。
- 来源格位、审核过程和下一素材需求保留在本清单；页面保留必要的概念图 / 历史图说明与集中使用提示，避免重复展示内部流程。
...
| 页面文案边界 | 使用“本站展示候选”“仅供本站展示，未开放复用授权”等准确表述；历史“官网内部展示”不表示访问受限，不能写“免费下载”“自由使用”“贴纸包”“素材 API”。 |
