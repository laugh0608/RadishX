...
状态：Gallery / Usage 实现、设计源同步、Chrome smoke、单张拆分准备清单、首批人工框选复核准备与授权 / 页面接入范围确认已完成
最后更新：2026-06-18
...
2026-06-18 追加首批单张表情授权与页面接入范围确认：当前只允许作为 RadishX 官网内部轻量展示候选，不作为下载素材、社交贴纸包、项目 Logo、首页主视觉或外部复用素材；生成单张 Web 文件前仍需项目所有者确认最终公开展示授权。
...
- 2026-06-18 已确认首批单张表情的页面接入范围；下一步如果继续推进，应先决定是否生成 Web 文件和是否新增 `/mascot` 单张预览区，而不是开放下载或外部分发。
...
- 当前授权边界只支持 RadishX 官网内部展示候选记录；未确认对外二次使用、商用复用、社交平台贴纸包或素材下载授权。
- 当前使用场景只考虑未来官网内部的轻量插图、状态反馈、FAQ、404 辅助、开发日志辅助视觉或 Mascot 页彩蛋候选，不作为独立品牌 Logo、项目图标或下载素材。
...
| 范围 | 当前结论 |
| --- | --- |
| 授权来源 | RadishX 仓库内原创候选素材；生成单张 Web 文件前仍需项目所有者确认最终公开展示授权。 |
| 允许页面位置 | `/mascot` 单张表情预览区候选、404 辅助视觉、FAQ / 状态反馈、开发日志辅助视觉。 |
| 禁止页面位置 | 首页首屏主视觉、四项目 Logo、项目图标、下载区、素材包入口、社交贴纸包入口、商业宣传授权页。 |
| 文件生成边界 | 授权确认后才可生成 `public/images/mascot/expressions/*-web.jpg`；缩略图只在页面性能或网格浏览真实需要时生成。 |
...
- 如果未来确认启动单张展示，先基于本页计划框选和接入范围决定是否生成单张 Web 文件，再同步数据层、页面文案和设计源。
