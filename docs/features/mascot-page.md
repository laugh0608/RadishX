...
Mascot 页已接入首批确认图片，包含原始形象、可爱Q版安全设定图、可爱Q版站姿图、虚拟形象完全体设定图、完全体站姿图和 Gallery 图片。页面暂不提供下载入口，也不声明素材可自由复用。
...
- 桌面视口检查 `/mascot`。
- 390px 移动端检查 `/mascot`。
- 检查 Mascot 页面图片加载成功，并确认新增 3 张公开 Web 资源返回 `200`。
