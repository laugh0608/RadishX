...
本文件保留 Gallery / Usage、首批单张表情与 craft Phase 4 的历史记录。后续公开文案与信息层级整理见 [官网质量与对外表达收口](site-quality-and-public-content.md)。`internal-preview` 是历史素材状态值，表示仅限本站展示的候选用途，不代表网页或文件存在访问控制；具体术语见 [素材治理](../assets/materials.md)。
...
- 原始形象用于图标、小印章和形态起点。
- 可爱Q版站姿图保留用于 Mascot 页形态展示；作为首页中心视觉锚点属于历史 craft 实现，灰玉首页首帧改用 Radish 已审核角色图，收束舞台使用透明完全体立绘。
...
- Pencil v1.2 craft 桌面与独立移动画板已包含 Gallery 状态 / 边界 / 后续处理、首批单张表情候选预览区和 Usage 四组边界。
...
- 已完成首批单张表情授权与页面接入范围确认：允许位置只限 `/mascot` 单张预览候选、404 辅助、FAQ / 状态反馈和开发日志辅助视觉；禁止进入下载区、素材包、社交贴纸包、首页首屏或五项目标识。
...
- 如后续继续调整单张表情展示区，需同步 v1.2 craft 的 Mascot 桌面与移动画板并执行 `snapshot_layout`。
