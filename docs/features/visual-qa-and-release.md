...
状态：保留历史 QA / 发布证据；新增验证补漏进入官网质量收口目标
最后更新：2026-09-06
...
- 本文历史记录仅对对应日期、版本、目标 URL 和视口组合有效；当时的“本轮”“待补”不代表新的执行指令。
- 2026-09-06 仅重跑构建、本地发布与协作入口检查，并进行源码审阅和定向函数 / 色值检查；未重跑 HTTP、浏览器或线上验收。详见 [2026-W36](../devlogs/2026-W36.md)。
- 构建证明代码与产物可生成；HTTP smoke 证明响应与外壳基本可用；浏览器检查才覆盖正文、解码、交互与布局。三类证据不能相互替代。
- `/lex` 本地 sitemap 检查遗漏、路由正文覆盖、404 响应语义和浏览器脚本归档的待办统一见 [官网质量收口](site-quality-and-public-content.md)。尚未建立的脚本不作为已有验证入口。
- 原始 HTML metadata 与 JavaScript 更新后的 metadata 分别检查；现有浏览器 metadata 记录不能证明逐路由静态 SEO 输出正确。

## 历史背景

以下保留既有本地响应式、可访问性与线上检查经过，最新实现与发布状态以 [当前规划](../planning/current.md) 为准。
